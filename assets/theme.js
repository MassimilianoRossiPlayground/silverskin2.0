...
(function($){
      ...
      _onSelectChange: function() {
        ...
        if(document.getElementsByClassName("wcp_vd_table").length!=0 && document.getElementById("vdtable") !=null){	
            ...
            var template = Handlebars.compile(source);
                if( document.getElementsByClassName("wcp_vd_table")[0] != undefined 
                    && 
                    document.getElementsByClassName("wcp_vd_table")[0].innerText != "" 
                    && 
                    document.getElementsByClassName("wcp_vd_table")[0].innerText.indexOf("\n") == -1 
                    ){
                    console.log(document.getElementsByClassName("wcp_vd_table")[0].innerText)
      ...
      initEventListeners: function() {
        ...
        $('body').on('added.ajaxProduct', function() {
          theme.cart.getCart().then(function(cart) {
            ...
            $('.cart-link__bubble').text($(".js-qty__num").val());
          }.bind(this));
        }.bind(this));
      },
  ...
  theme.headerNav = (function() {
    ...
    function stickyHeaderSetup() {
      if (!config.stickyInitialized) {
        ...
        if(window.innerWidth>768){
          ...
          $window.on('scroll' + config.namespace, $.throttle(15, stickyHeaderScrollMob));
        }
      }
    ...
    function stickyHeaderScrollMob(){
       if (config.forceStopSticky) {
        return;
      }
      ...
      if (scroll < config.lastScroll){
        ...
        config.stickyActive = true;
        $siteHeader.addClass(config.stickyClass).addClass(config.openTransitionClass);
        
      }else{

         $siteHeader.removeClass(config.stickyClass);
